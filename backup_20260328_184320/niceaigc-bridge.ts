...
export class NiceAIGCBridgeAdapter {
  ...
  private async runCommand(command: string, timeout: number = 60000): Promise<{ stdout: string; stderr: string }> {
    ...
    // Use forward slashes for Windows path compatibility with child_process
    const openclawPath = 'E:/AI_projects/openclaw';
    const fullCommand = `cd "${openclawPath}" && node index.js ${command}`;
    ...
    try {
      ...
      if (error.killed && error.signal === 'SIGTERM') {
        logger.warn(`[NiceAIGC] Command timed out but may have succeeded: ${fullCommand}`);
        // Return any stdout that was collected before timeout
        return { 
          stdout: error.stdout || '', 
          stderr: error.stderr || '' 
        };
      }
      logger.error(`[NiceAIGC] Command failed: ${fullCommand}`);
      logger.error(`[NiceAIGC] Error: ${error.message}`);
      throw error;
    }
  }
