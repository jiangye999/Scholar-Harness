...
const BASE_URL = String(
  process.env.SCHOLAR_HARNESS_META_URL
...
async function apiJson(endpoint: string, init: RequestInit = {}): Promise<JsonObject> {
  const response = await apiResponse(endpoint, init);
  return asObject(await response.json());
}

async function postJson(endpoint: string, body: JsonObject): Promise<JsonObject> {
...
function unwrapData(response: JsonObject): JsonObject {
  return asObject(response.data);
}

function responsePreview(value: unknown): unknown {
  const serialized = JSON.stringify(value, null, 2);
  if (serialized.length <= MAX_RESPONSE_CHARS) return value;
  return {
    truncated: true,
...
function toolResult(value: unknown, isError = false): JsonObject {
...
async function readLocalFile(filePath: string): Promise<{ absolutePath: string; name: string; buffer: Buffer }> {
...
async function writeBinaryResponse(response: Response, outputPath: string): Promise<JsonObject> {
...
function summarizeMetaList(payload: JsonObject): JsonObject {
...
function summarizeRun(run: JsonObject, rArtifacts?: JsonObject): JsonObject {
...
async function executeRArtifacts(
  userId: string,
  run: JsonObject,
  timeoutMs?: number,
): Promise<JsonObject> {
...
async function handleTool(name: string, args: JsonObject): Promise<JsonObject> {
  const userId = await getUserId(args);

  switch (name) {
    case 'meta_health': {
      const health = await apiJson('/health');
      let rPlugin: JsonObject;
      try {
        rPlugin = await apiJson('/api/r-code/plugin/status');
      } catch (error) {
        rPlugin = { success: false, error: (error as Error).message };
      }
    ...
    case 'meta_plan_analysis': {
      ...
      let rArtifacts: JsonObject | undefined;
    ...
    case 'meta_run_analysis': {
      ...
      if (Object.keys(config).length === 0) throw new Error('config must be a non-empty object');
    ...
    case 'meta_get_results': {
      ...
      if (typeof args.analysisId === 'string' && args.analysisId.trim()) params.set('analysisId', args.analysisId.trim());
      if (typeof args.conversationId === 'string' && args.conversationId.trim()) params.set('conversationId', args.conversationId.trim());
      const response = await apiJson(`/api/meta-analysis/writing-context?${params.toString()}`);
      const context = unwrapData(response);
      if (args.detail === 'full') return toolResult(context);
      return toolResult({
    ...
    case 'pdfwiki_obsidian_search': {
      ...
      const payload = await apiJson(
        `/api/pdf-wiki/obsidian/search?userId=${encodeURIComponent(userId)}&q=${encodeURIComponent(query)}&limit=${limit}`,
      );
      return toolResult(payload);
    }
...
lines.on('line', line => {
  ...
  try {
    ...
    send({ jsonrpc: '2.0', id: null, error: { code: -32700, message: `Parse error: ${(error as Error).message}` } });
    return;
  }
