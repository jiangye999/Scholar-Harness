/**
 * NiceAIGC 桥接插件 - HTTP 服务器
 * 
 * 启动后提供 REST API 供你的后端调用
 * 
 * 使用方式：
 *   node server.js --port 8765
 * 
 * API 端点：
 *   POST /chat - 发送消息
 *   POST /chat/file - 发送消息并保存文件
 *   GET /health - 健康检查
 */

import express from 'express';
import cors from 'cors';
import NiceAIGCBridge from './bridge.js';
import { exec } from 'child_process';
import { promisify } from 'util';
import { readFile } from 'fs/promises';
import { dirname, join } from 'path';
import { fileURLToPath } from 'url';
import { existsSync } from 'fs';

const execAsync = promisify(exec);
const __dirname = dirname(fileURLToPath(import.meta.url));

/**
 * 获取 OpenClaw 路径（跨平台兼容）
 */
function getOpenclawPath() {
  // 1. 优先使用环境变量
  if (process.env.OPENCLAW_PATH || process.env.OPENCLAW_DIR) {
    return process.env.OPENCLAW_PATH || process.env.OPENCLAW_DIR;
  }
  
  // 2. Electron 打包环境
  if (process.resourcesPath) {
    const packagedPath = join(process.resourcesPath, 'openclaw');
    if (existsSync(packagedPath)) {
      return packagedPath;
    }
  }
  
  // 3. 开发环境：相对于项目根目录
  return join(process.cwd(), 'openclaw');
}

const app = express();
const PORT = process.env.PORT || 8765;

// 中间件
app.use(cors());
app.use(express.json({ limit: '10mb' }));
app.use(express.text({ limit: '10mb' }));

// 初始化桥接器
const bridge = new NiceAIGCBridge();

// 配置缓存
let config = null;
async function loadConfig() {
  if (!config) {
    const configData = await readFile(join(__dirname, 'config.json'), 'utf-8');
    config = JSON.parse(configData);
  }
  return config;
}

// 健康检查
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'niceaigc-bridge', timestamp: new Date().toISOString() });
});

/**
 * GET /newchat
 * 在大牛马页面创建新聊天
 * 用于 ScholarHarness 新建会话时同步创建新聊天
 */
app.get('/newchat', async (req, res) => {
  try {
    const cfg = await loadConfig();
    const openclawPath = getOpenclawPath();
    const chatUrl = cfg.niceaigc.chat_url;
    const profile = cfg.browser.profile;
    
    console.log(`[NewChat] 在大牛马页面创建新聊天: ${chatUrl}`);
    
    const { stdout, stderr } = await execAsync(
      `node index.js browser --action open --url "${chatUrl}" --profile ${profile} --new-window`,
      { cwd: openclawPath, timeout: 15000 }
    );
    
    console.log(`[NewChat] 完成`);
    res.json({ 
      success: true, 
      message: '新聊天已创建',
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error(`[NewChat] 错误：${error.message}`);
    res.json({ 
      success: false, 
      error: error.message 
    });
  }
});

/**
 * GET /refresh
 * 刷新大牛马页面
 * 用于页面卡住时恢复响应
 */
app.get('/refresh', async (req, res) => {
  try {
    const cfg = await loadConfig();
    const openclawPath = getOpenclawPath();
    const chatUrl = cfg.niceaigc.chat_url;
    const profile = cfg.browser.profile;
    
    console.log(`[Refresh] 刷新大牛马页面`);
    
    const { stdout, stderr } = await execAsync(
      `node index.js browser --action open --url "${chatUrl}" --profile ${profile}`,
      { cwd: openclawPath, timeout: 15000 }
    );
    
    console.log(`[Refresh] 完成`);
    res.json({ 
      success: true, 
      message: '页面已刷新',
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error(`[Refresh] 错误：${error.message}`);
    res.json({ 
      success: false, 
      error: error.message 
    });
  }
});

/**
 * POST /chat/stream
 * 流式发送消息到 NiceAIGC
 * 支持 SSE 流式响应
 */
app.post('/chat/stream', async (req, res) => {
  try {
    const { message, mode } = req.body;
    
    if (!message) {
      return res.status(400).json({ 
        success: false, 
        error: '缺少 message 参数' 
      });
    }
    
    res.setHeader('Content-Type', 'text/event-stream');
    res.setHeader('Cache-Control', 'no-cache');
    res.setHeader('Connection', 'keep-alive');
    
    const cfg = await loadConfig();
    const openclawPath = getOpenclawPath();
    const chatUrl = cfg.niceaigc.chat_url;
    const profile = cfg.browser.profile;
    const waitMs = cfg.browser.wait_for_response_ms || 240000;
    
    console.log(`[Stream] 开始流式聊天 | 消息长度：${message.length}`);
    
    let finalMessage = message;
    
    const escapedMessage = finalMessage.replace(/"/g, '\\"').replace(/\n/g, '\\n');
    
    const child = require('child_process').spawn('node', [
      'index.js', 'browser', '--action', 'chat',
      '--url', chatUrl,
      '--profile', profile,
      '--text', escapedMessage,
      '--wait', String(waitMs)
    ], { cwd: openclawPath });
    
    let fullResponse = '';
    
    child.stdout.on('data', (data) => {
      const text = data.toString();
      const lines = text.split('\n').filter(line => line.trim());
      
      for (const line of lines) {
        try {
          const parsed = JSON.parse(line);
          if (parsed.type === 'start') {
            fullResponse = parsed.content || '';
            res.write(`data: ${JSON.stringify({ type: 'start', content: fullResponse })}\n\n`);
          } else if (parsed.type === 'chunk') {
            const newPart = parsed.content || '';
            if (newPart) {
              fullResponse += newPart;
              res.write(`data: ${JSON.stringify({ type: 'chunk', content: newPart })}\n\n`);
            }
          } else if (parsed.type === 'end') {
            res.write(`data: ${JSON.stringify({ type: 'done', content: fullResponse })}\n\n`);
          }
        } catch (e) {
          // 非 JSON 行，忽略
        }
      }
    });
    
    child.stderr.on('data', (data) => {
      console.error(`[Stream-Error] ${data.toString()}`);
    });
    
    child.on('close', (code) => {
      console.log(`[Stream] 完成 | 退出码：${code} | 响应长度：${fullResponse.length}`);
      res.write(`data: ${JSON.stringify({ type: 'complete', content: fullResponse })}\n\n`);
      res.end();
    });
    
    child.on('error', (err) => {
      console.error(`[Stream] 进程错误：${err.message}`);
      res.write(`data: ${JSON.stringify({ type: 'error', error: err.message })}\n\n`);
      res.end();
    });
    
    req.on('close', () => {
      child.kill();
    });
    
  } catch (error) {
    console.error(`[Stream] 错误：${error.message}`);
    res.write(`data: ${JSON.stringify({ type: 'error', error: error.message })}\n\n`);
    res.end();
  }
});

/**
 * POST /chat
 * 发送消息到 NiceAIGC
 * 
 * 请求体：
 * {
 *   "message": "请帮我润色这段论文...",
 *   "mode": "browser"  // 可选，默认 browser
 * }
 * 
 * 响应：
 * {
 *   "response": "AI 的回复内容",
 *   "success": true,
 *   "timestamp": "..."
 * }
 */
app.post('/chat', async (req, res) => {
  try {
    const { message, mode } = req.body;
    
    if (!message) {
      return res.status(400).json({ 
        success: false, 
        error: '缺少 message 参数' 
      });
    }
    
    console.log(`[Server] 收到请求 | 消息长度：${message.length}`);
    
    const response = await bridge.sendMessage(message, mode);
    
    res.json({
      success: true,
      response,
      timestamp: new Date().toISOString(),
      mode: mode || 'browser'
    });
    
  } catch (error) {
    console.error(`[Server] 错误：${error.message}`);
    res.status(500).json({
      success: false,
      error: error.message
    });
  }
});

/**
 * 获取默认输出文件路径（跨平台兼容）
 */
function getDefaultOutputPath() {
  const dataDir = join(process.cwd(), 'data');
  return join(dataDir, 'niceaigc_response.txt');
}

/**
 * POST /chat/file
 * 发送消息并保存结果到文件
 * 
 * 请求体：
 * {
 *   "message": "请帮我润色...",
 *   "outputPath": "data/response.txt"  // 可选，相对路径
 * }
 * 
 * 响应：
 * {
 *   "response": "AI 的回复内容",
 *   "savedTo": "data/response.txt",
 *   "success": true
 * }
 */
app.post('/chat/file', async (req, res) => {
  try {
    const { message, outputPath } = req.body;
    
    if (!message) {
      return res.status(400).json({ 
        success: false, 
        error: '缺少 message 参数' 
      });
    }
    
    console.log(`[Server] 收到文件保存请求 | 消息长度：${message.length}`);
    
    const response = await bridge.sendAndSave(message, outputPath);
    
    res.json({
      success: true,
      response,
      savedTo: outputPath || getDefaultOutputPath(),
      timestamp: new Date().toISOString()
    });
    
  } catch (error) {
    console.error(`[Server] 错误：${error.message}`);
    res.status(500).json({
      success: false,
      error: error.message
    });
  }
});

/**
 * POST /chat/batch
 * 批量处理多条消息
 * 
 * 请求体：
 * {
 *   "messages": ["消息 1", "消息 2", ...],
 *   "delay": 2000  // 每条消息之间的延迟（毫秒），可选
 * }
 * 
 * 响应：
 * {
 *   "results": [
 *     { "message": "消息 1", "response": "...", "success": true },
 *     { "message": "消息 2", "response": "...", "success": false, "error": "..." }
 *   ],
 *   "total": 2,
 *   "success": 1,
 *   "failed": 1
 * }
 */
app.post('/chat/batch', async (req, res) => {
  try {
    const { messages, delay = 2000 } = req.body;
    
    if (!Array.isArray(messages) || messages.length === 0) {
      return res.status(400).json({ 
        success: false, 
        error: 'messages 必须是非空数组' 
      });
    }
    
    console.log(`[Server] 收到批量请求 | 消息数：${messages.length}`);
    
    const results = [];
    let successCount = 0;
    let failCount = 0;
    
    for (let i = 0; i < messages.length; i++) {
      const message = messages[i];
      
      try {
        const response = await bridge.sendMessage(message);
        results.push({
          index: i,
          message,
          response,
          success: true
        });
        successCount++;
      } catch (error) {
        results.push({
          index: i,
          message,
          success: false,
          error: error.message
        });
        failCount++;
      }
      
      // 延迟（避免请求过快）
      if (i < messages.length - 1 && delay > 0) {
        await bridge.sleep(delay);
      }
    }
    
    res.json({
      success: true,
      results,
      total: messages.length,
      success: successCount,
      failed: failCount,
      timestamp: new Date().toISOString()
    });
    
  } catch (error) {
    console.error(`[Server] 错误：${error.message}`);
    res.status(500).json({
      success: false,
      error: error.message
    });
  }
});

// 启动服务器
app.listen(PORT, () => {
  console.log('');
  console.log('='.repeat(60));
  console.log('   NiceAIGC 桥接服务已启动');
  console.log('='.repeat(60));
  console.log(`   端口：${PORT}`);
  console.log(`   地址：http://localhost:${PORT}`);
  console.log('');
  console.log('可用端点:');
  console.log('   GET  /health       - 健康检查');
  console.log('   GET  /newchat      - 创建新聊天（同步ScholarHarness新会话）');
  console.log('   GET  /refresh      - 刷新大牛马页面');
  console.log('   POST /chat         - 发送消息');
  console.log('   POST /chat/stream  - 流式发送消息');
  console.log('   POST /chat/file    - 发送消息并保存文件');
  console.log('   POST /chat/batch   - 批量处理');
  console.log('');
  console.log('示例:');
  console.log('   curl http://localhost:' + PORT + '/health');
  console.log('   curl -X POST http://localhost:' + PORT + '/chat \\');
  console.log('     -H "Content-Type: application/json" \\');
  console.log('     -d \'{"message": "请帮我润色这段论文..."}\'');
  console.log('='.repeat(60));
  console.log('');
});
