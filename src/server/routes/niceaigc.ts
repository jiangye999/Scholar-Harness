...
router.post('/chat', async (req, res) => {
  try {
    ...
    const { message, context = {}, options = {}, stream = false } = req.body;
    ...
    logger.info('[Debug] Context received:', {
      ...
      writingProgress: context.memory?.writingProgress,
      completedChapters: context.memory?.completedChapters,
      hasLiterature: !!context.literature,
      literatureCount: context.literature?.count,
      hasJournalStyle: !!context.journalStyle,
      hasWritingSkill: !!context.writingSkill
    });
    ...
    if (stream) {
      ...
      try {
        ...
        // Send final response
        res.write(`data: ${JSON.stringify({ type: 'complete', response, provider: 'niceaigc' })}\n\n`);
        res.end();
      } catch (error) {
...
function buildEnrichedMessage(message: string, context: any): string {
  ...
  // 调试：打印最终消息长度
  logger.info(`[Debug] Final enriched message length: ${enrichedPrompt.length} characters`);
  logger.info('[Debug] Message preview (first 500 chars):', enrichedPrompt.substring(0, 500));
