...
class NiceAIGCBridge:
    ...
    def send_and_save(self, message: str, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            output_path = self.config['local'].get('output_file', '/tmp/niceaigc_response.txt')
        ...
        print(f"[Bridge] 结果已保存到：{output_path}")
        return response
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, choices=['api', 'browser', 'auto'], 
                        help='运行模式（覆盖配置）')
